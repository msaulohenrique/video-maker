...
async function robot() {
  ...
  async function authenticateWithOAuth() {
    ...
    async function startWebServer() {
      return new Promise((resolve, reject) => {
        ...
        const server = app.listen(port, () => {
          console.log(`> [youtube-robot] Ouvindo em http://localhost:${port}`)
    ...
    function requestUserConsent(OAuthClient) {
      ...
      console.log(`> [youtube-robot] Por favor, dê o seu consentimento: ${consentUrl}`)
    }
    ...
    async function waitForGoogleCallback(webServer) {
      return new Promise((resolve, reject) => {
        ...
        webServer.app.get('/oauth2callback', (req, res) => {
          ...
          res.send('<h1>Obrigado!</h1><p>Agora feche esta guia.</p>')
          resolve(authCode)
        })
      })
    }
  ...
  async function uploadVideo(content) {
    ...
    console.log('> [youtube-robot] Começando a enviar o vídeo para o YouTube')
    const youtubeResponse = await youtube.videos.insert(requestParameters, {
      onUploadProgress: onUploadProgress
    })

    console.log(`> [youtube-robot] Vídeo disponível em: https://youtu.be/${youtubeResponse.data.id}`)
    return youtubeResponse.data

    function onUploadProgress(event) {
      const progress = Math.round( (event.bytesRead / videoFileSize) * 100 )
      console.log(`> [youtube-robot] ${progress}% concluído`)
    }
  ...
  async function uploadThumbnail(videoInformation) {
    ...
    console.log(`> [youtube-robot] Miniatura enviada!`)
  }
